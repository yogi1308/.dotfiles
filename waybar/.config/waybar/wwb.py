...
LATITUDE, LONGITUDE, CITY, STATE = get_location()

print(LATITUDE, LONGITUDE, CITY, STATE)
...
def waybar_entry(label: str, content: str, indent: int = 2, label_width: int = 9):
    ...
    label_with_whitespace = label.ljust(label_width)
    return f'{" " * indent}{colorize(label_with_whitespace, BLUE)}  {content}\n'
...
def waybar_current(data: dict) -> str:
    ...
    output += waybar_entry('humidity', colorize(f'{humidity} % RH', YELLOW))
    output += waybar_entry('wind',     f'{colorize(f"{wind_speed} m/s", ORANGE)} {colorize(f"({wind_direction})", GRAY)}')
    output += waybar_entry('rain',     colorize(f"{rainfall} mm", YELLOW))
    return output
...
def waybar():
    ...
    tooltip = colorize(f'{CITY}, {STATE}', BLUE) + '\n' + \
              colorize(f'current weather', GREEN) + '\n' + current + '\n' + \
