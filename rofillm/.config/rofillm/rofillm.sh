...
if [[ -z "$@" ]]; then
    ...
    WRAPPED=$(echo "$ANSWER" | tr -d '`' | fold -s -w 80)
    
    printf '%s\n%s\n%s' "$HEADER" "$SEPARATOR" "$WRAPPED" | \
        rofi -dmenu -p "" -no-custom \
        -theme-str "window {width: 55%;} listview {lines: $LINES; scrollbar: true; border: 0px;} inputbar {enabled: false;} scrollbar {width: 4px;}" | wl-copy
